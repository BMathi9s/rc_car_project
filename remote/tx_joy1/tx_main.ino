...
// For fpv camera (joystick right)
#define JOYSTICK2_X_PIN 25
#define JOYSTICK2_Y_PIN 33
#define JOYSTICK2_SWITCH_PIN 26 // active low


// For marshmallow cannon data
#define SPEED_POT_PIN 13
#define LEFT_SPIN_BUTTON_PIN 34
#define RIGHT_SPIN_BUTTON_PIN 35
...
struct {
  ...
  byte cannonData[3]; // [speedPot, leftButton, rightButton]
} package;
...
void setup() {
  ...
  radio.setPayloadSize(9);  // this sets the package size to 6 bytes (default, max: 32 bytes) - 6 bytes for joystick values
  ...
  pinMode(LEFT_SPIN_BUTTON_PIN, INPUT_PULLUP);
  // attachInterrupt(digitalPinToInterrupt(LEFT_SPIN_BUTTON_PIN), handleLeftButtonInterrupt, RISING);
  pinMode(RIGHT_SPIN_BUTTON_PIN, INPUT_PULLUP);
  // attachInterrupt(digitalPinToInterrupt(RIGHT_SPIN_BUTTON_PIN), handleLeftButtonInterrupt, RISING);
...
void loop() {
  ...
  // for marshmallow cannon
  int rawPot = analogRead(SPEED_POT_PIN);
  leftButtonState = digitalRead(LEFT_SPIN_BUTTON_PIN);
  rightButtonState = digitalRead(RIGHT_SPIN_BUTTON_PIN);

  package.cannonData[0] = map(rawPot, 0, 4095, 0, 255);
